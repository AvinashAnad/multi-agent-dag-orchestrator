...
MAX_TOOL_HOPS = 15  # raised from 6; bulk index jobs (e.g. 5 files) need more hops
...
async def run_with_tools(*, prompt: str, tools_payload: list[dict],
    ...
    try:
        ...
        # stdio_client uses asyncio.TaskGroup internally; if the MCP subprocess
        # crashes it raises an ExceptionGroup (possibly nested). Recursively
        # unwrap to the leaf exceptions so the actual error text (e.g. "503")
        # survives into the RuntimeError message and classify_failure() can read it.
        def _leaf_msgs(exc) -> list[str]:
            if hasattr(exc, "exceptions"):          # ExceptionGroup
                msgs = []
                for inner in exc.exceptions:
                    msgs.extend(_leaf_msgs(inner))
                return msgs or [str(exc)]
            return [str(exc)]

        causes = _leaf_msgs(eg)
        raise RuntimeError(f"MCP session failed: {'; '.join(causes)}") from eg
